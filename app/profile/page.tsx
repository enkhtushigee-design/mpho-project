    ...
    return (
      <main className="min-h-screen bg-slate-50 flex items-center justify-center font-sans">
        <div className="text-center">
          <p className="text-slate-400 font-black uppercase tracking-widest text-sm mb-6">Нэвтрэх шаардлагатай</p>
          <Link href="/login" className="px-8 py-4 bg-slate-950 text-white rounded-2xl font-bold text-sm hover:bg-blue-600 transition-all">
            Нэвтрэх
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-slate-50 font-sans pb-20">
      ...
      <div className="max-w-2xl mx-auto px-6 pt-10">
        ...
        <div className="bg-slate-950 rounded-[40px] p-8 text-white mb-8 shadow-2xl">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">ТАНЫ MPHO ID</p>
          <p className="text-5xl font-[1000] tracking-widest mb-4">{user.student_id}</p>
        ...
        <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm p-8 space-y-4">
          <h2 className="text-lg font-[1000] uppercase tracking-tighter text-slate-950 italic mb-2">Мэдээлэл засах</h2>
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Сургууль</label>
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">И-мэйл</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              className="w-full p-4 bg-slate-50 rounded-2xl font-bold text-slate-900 outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
          </div>

          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Шинэ нууц үг (заавал биш)</label>
          ...
          {error && <div className="bg-red-50 border border-red-200 rounded-2xl p-4"><p className="text-red-600 font-bold text-sm">{error}</p></div>}
          {success && <div className="bg-green-50 border border-green-200 rounded-2xl p-4"><p className="text-green-600 font-bold text-sm">Амжилттай хадгалагдлаа!</p></div>}
          ...
          <button onClick={() => { localStorage.removeItem("mpho_user"); window.location.href = "/"; }}
            className="w-full py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl font-[1000] text-sm transition-all">
            Гарах
          </button>
        </div>
      </div>
